...
| PAISÀ modern-Italian curriculum source | Not yet acquired; approved for local acquisition only | [`data/metadata/paisa_attribution.md`](data/metadata/paisa_attribution.md) and the committed [`release activation audit`](reports/paisa_release_activation_audit.json). The corpus text, derived token files, and PAISÀ-derived checkpoints remain local because the source corpus is CC BY-NC-SA. |
