...
| PAISÀ modern-Italian curriculum source, v1 | Locally acquired and split; not public repository data | [`data/metadata/paisa_attribution.md`](data/metadata/paisa_attribution.md), the committed [`release activation audit`](reports/paisa_release_activation_audit.json), [`aggregate local-build report`](reports/paisa_modern_italian_v1_build_report.json), and the [`fixed rescue curriculum`](docs/paisa_historical_rescue_curriculum.md). The corpus text, document inventory, derived token files, and PAISÀ-derived checkpoints remain local because the source corpus is CC BY-NC-SA. |
