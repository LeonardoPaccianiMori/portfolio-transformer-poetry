...
compares from-scratch training with parameter-efficient adaptation of open
Minerva language models. The work was performed under a 6 GiB laptop-GPU target,
with one rented 48 GiB GPU used for unquantized Minerva 7B LoRA training.
...
substantially improved topic continuity, grammar, and collapse rates. The
selected 7B adapter reached 20/20 controlled forms, 8/20 generally grammatical
outputs, 20/20 topic-continuous outputs, and 5/20 severe collapses. It was the
strongest system but still failed the fixed acceptable-quality gate. A later
Minerva-guided reward gate exposed inverse correlation with collapse, so the
predeclared DPO and GRPO branches were cancelled.
...
The final generation protocol fixes prompts, seeds, temperature, top-k, token
ceiling, line target, and special-token suppression. Reports include every
sample, not only favorable examples.

Automatic checks cover exact opening preservation, non-empty line count,
boundary-marker leakage, repetition diagnostics, and character-level overlap
with training poems. Human qualitative review uses literal binary decisions for
generally grammatical Italian, seven-line topic continuity, and severe
collapse. These dimensions are reported separately so strong form control
cannot conceal language failure.
...
## 8. Reinforcement And Preference Learning Decision
...
The gate therefore failed and both DPO and GRPO were cancelled. This negative
result is more useful than running reinforcement learning merely to claim the
technique: it demonstrates why reward validation is a prerequisite.
...
demo. The completed repository passes all 687 CPU tests, including seven tests
for demo request validation and required static assets.
...
- Sonnet metre and rhyme were not evaluated.
- The strongest system inherits unknown external-pretraining overlap.
- Historical orthography complicates grammatical review.
- Human qualitative sample sizes are intentionally bounded and not a general
  benchmark of Italian generation.
...
Pretrained Italian models delivered large improvements with a small trainable
parameter surface. Staged historical and sonnet LoRA on Minerva 7B produced the
best outputs, but grammar and repetition remained below the declared standard.
