#!/usr/bin/env python3
"""Train the reasoning-trace LoRA from the derived corpus traces."""
...
from sonnet_training.reasoning_trace_sft import build_trace_examples
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "artifacts/local/reasoning_trace/training",
    )
...
def main() -> None:
    ...
    examples, skipped = build_trace_examples(
        cards,
        tokenizer,
    ...
    report = train_plan_following_sft(
        ...
        config=config,
        progress=lambda message: print(f"reasoning-trace-train | {message}", flush=True),
    )
    print(
        "reasoning-trace-train | complete "
