#!/usr/bin/env python3
"""Generate one autonomous arm for the self-play RFT evaluation."""
...
from sonnet_training.self_play_rft import (
    ARMS,
    generate_autonomous_validation,
)
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("autonomous generation requires one CUDA GPU")
    ...
    result = generate_autonomous_validation(
        model=model,
        tokenizer=tokenizer,
        prompts=prompts,
        arm=args.arm,
        ...
        progress=lambda message: print(f"autonomous | {message}", flush=True),
    )
    print(
        f"autonomous | complete arm={args.arm} "
