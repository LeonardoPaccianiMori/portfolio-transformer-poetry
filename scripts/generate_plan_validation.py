#!/usr/bin/env python3
"""Generate one plan-generator arm for evaluation."""
...
from sonnet_training.reasoning_trace_sft import PLAN_ARMS, generate_plan_validation


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--arm", choices=PLAN_ARMS, required=True)
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("plan generation requires one CUDA GPU")
    ...
    result = generate_plan_validation(
        ...
        progress=lambda message: print(f"plan-gen | {message}", flush=True),
    )
    print(
        f"plan-gen | complete arm={args.arm} "
