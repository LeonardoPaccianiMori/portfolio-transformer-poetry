...
def probe_gutenberg_sources(
    *,
    ...
    report = {
        ...
        "total_bpe_tokens": (
            sum(result.bpe_token_count or 0 for result in results)
...
def _probe_gutenberg_row(
    *,
    ...
    token_ids = tokenizer.encode(cleaned) if tokenizer is not None else None
    return GutenbergProbeResult(
        source_id=row.source_id,
        title=row.title,
        author=row.author,
        ...
        bpe_token_count=len(token_ids) if token_ids is not None else None,
        cleaning_notes=row.cleaning_notes,
    )
