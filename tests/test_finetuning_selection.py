...
def test_selection_manifest_contains_parent_architecture_and_lineage(tmp_path):
    ...
    assert selection["model_architecture"]["normalization_type"] == "layer_norm"
    assert selection["model_architecture"]["normalization_eps"] == 1e-5
    assert selection["model_architecture"]["position_encoding_type"] == (
        "learned_absolute"
    )
    assert selection["model_architecture"]["rope_theta"] == 10_000.0
    assert selection["model_architecture"]["feed_forward_type"] == "relu"
    assert selection["model_architecture"]["tie_token_embeddings"] is False
...
def test_selection_preserves_explicit_tied_embedding_architecture(tmp_path):
    run_dir = tmp_path / "runs" / "control"
    run_dir.mkdir(parents=True)
    write_json(
        run_dir / "config.json",
    ...
    assert selection["model_architecture"]["tie_token_embeddings"] is True


def test_selection_preserves_explicit_rms_norm_architecture(tmp_path):
    run_dir = tmp_path / "runs" / "control"
    run_dir.mkdir(parents=True)
    write_json(
        run_dir / "config.json",
    ...
    assert selection["model_architecture"]["normalization_type"] == "rms_norm"
    assert selection["model_architecture"]["normalization_eps"] == 1e-6


def test_selection_preserves_explicit_rope_architecture(tmp_path):
    run_dir = tmp_path / "runs" / "control"
    run_dir.mkdir(parents=True)
    write_json(
        run_dir / "config.json",
